...
function SubtaskDetail({ node, completedIds, onBack, handleToggleNode, isNodeInteractive }) {
  ...
  return (
    <div className="treeview-sidebar__detail">
      ...
      {mins ? <p className="treeview-sidebar__detail-time">~{mins} min</p> : null}
      ...
      {resources.length > 0 && (
        <>
          ...
          <ul className="treeview-sidebar__resources">
            {resources.map((r, i) => (
              <li key={i} className="treeview-sidebar__resource">
                {r}
              </li>
            ))}
          </ul>
...
function TreeViewForProject({ projectId, user }) {
  ...
  if (!project?.reactFlowData) {
    return (
      <div className="treeview-page__state">No roadmap yet. Generate one from the project wizard first.</div>
    )
  }
...
export default function TreeView() {
  ...
  return (
    <div className="treeview-page viewer-page">
      {body}
      <BottomNav />
    </div>
  )
